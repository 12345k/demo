...
def loadVggFaceModel():
    ...
    model.load_weights('E:/code/face_recognition/face_detection/vgg_face_weights.h5')
...
employee_pictures = "E:/code/face_recognition/face_detection/images"
global employees
employees = dict()
def training():
    for file in listdir(employee_pictures):
        employee, extension = file.split(".")
        employees[employee] = model.predict(preprocess_image('E:/code/face_recognition/face_detection/images/%s.jpg' % (employee)))[0,:]
    ...
    for (x,y,w,h) in faces:
        if w > 130:
            ...
            for i in employees:
                ...
                if(similarity < 0.30):
                    cv2.putText(img, employee_name, (int(x+6), int(y+h-6)), cv2.FONT_HERSHEY_SIMPLEX, 1, color, 2)
            ...
            if(found == 0): #if found image is not in employee database
                cv2.putText(img, 'unknown', (int(x+6), int(y+h-6)), cv2.FONT_HERSHEY_SIMPLEX, 1, color, 2)
    return img
